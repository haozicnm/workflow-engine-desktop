...
interface Step {
  ...
  runCondition?: StepCondition   // 去中心化条件路由
}
...
struct Step {
    ...
    step_type: String,      // 对应前端 type（parser 透传类型名（v8））
...
| 前端字段 | 后端字段 | 转换规则 |
|---------|---------|---------|
| `type: "browser"` | `step_type: "browser"` | 类型名透传（v8） |
...
浏览器容器 (BrowserContainerNode)
    ...
    │
    ├─ input_ports: { "登录按钮_in": "admin" }  ← 上游步骤传入
    │
    ├─ 输出：output_ports = {
...
condition_group 评估
    ...
    │
    ├─ combinator = "and" → 全部为真 → 输出 branch="true"
    │                       任一为假 → 输出 branch="false"
    │
    └─ combinator = "or"  → 任一为真 → 输出 branch="true"
                           全部为假 → 输出 branch="false"
```

> 条件节点不再内嵌 thenSteps/elseSteps。下游步骤通过 `runCondition` 自行声明依赖哪个逻辑步骤的 true/false 分支，实现去中心化条件路由。

---

## 8. 条件执行模式（已替代 DAG）

> **DAG 执行模式已于 2026-05-06 移除。** 当前采用去中心化条件路由：每个步骤通过 `runCondition` 声明依赖哪个逻辑步骤的 true/false 分支，调度器在执行前检查条件。无需拓扑排序或并行调度。
...
App.vue 通过 `currentView` 状态手动切换页面（不使用 Vue Router）：

```
'welcome'  → Dashboard.vue     (工作流列表)
'editor'   → Editor.vue        (工作流编辑器)
