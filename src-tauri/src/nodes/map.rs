...
impl NodeExecutor for MapNode {
    ...
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        ...
        let source_raw = config.get("source")
        ...
        // 逐个映射
        let results: Vec<serde_json::Value> = items.iter().enumerate().map(|(i, item)| {
...
fn resolve_array(source: &str, ctx: &ExecutionContext) -> Result<Vec<serde_json::Value>> {
    let path = source.strip_prefix("output.")
        .ok_or_else(|| anyhow!("source 必须以 'output.' 开头，如 'output.step_id.results'"))?;

    let parts: Vec<&str> = path.split('.').collect();
    let step_id = parts[0];

    let mut current = ctx.get_output(step_id)
