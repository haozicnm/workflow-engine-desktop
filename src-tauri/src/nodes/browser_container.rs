...
/// 执行导航类动作：navigate, back, forward, reload, new_page, close_page, switch_page
/// 返回 Some(value) 如果动作产出数据，None 如果无输出
async fn execute_nav_action(action: &ContainerAction) -> Result<Option<Value>> {
    match action.action_type.as_str() {
        ...
        _ => unreachable!("execute_nav_action called with unsupported type: {}", action.action_type),
    }
}

// ─── 子函数：交互类动作 ───

/// 执行交互类动作：click, double_click, input/fill, select, check, hover, keyboard,
/// drag_to, context_menu, scroll, scroll_to_element
/// 返回 Some(value) 如果动作产出数据，None 如果无输出
async fn execute_interact_action(
    ...
    match action.action_type.as_str() {
        ...
        _ => unreachable!("execute_interact_action called with unsupported type: {}", action.action_type),
    }
}

// ─── 子函数：提取类动作 ───

/// 执行提取类动作：extract, extract_table, extract_links, get_title, current_url,
/// pages, snapshot, screenshot, cookies, evaluate
/// 返回 Some(value) 如果动作产出数据，None 如果无输出
async fn execute_extract_action(action: &ContainerAction) -> Result<Option<Value>> {
    match action.action_type.as_str() {
        "extract" => {
            let selector = action
                .config
        ...
        _ => unreachable!("execute_extract_action called with unsupported type: {}", action.action_type),
    }
}

// ─── 子函数：等待类动作 ───

/// 执行等待类动作：wait, wait_network_idle, wait_load_state, wait_url_contains
/// 返回 Some(value) 如果动作产出数据，None 如果无输出
async fn execute_wait_action(action: &ContainerAction) -> Result<Option<Value>> {
    match action.action_type.as_str() {
        "wait" => {
        ...
        _ => unreachable!("execute_wait_action called with unsupported type: {}", action.action_type),
    }
}

// ─── 子函数：工具类动作 ───

/// 执行工具类动作：download, upload, pdf, set_headers, switch_frame, handle_dialog, verify
/// 返回 Some(value) 如果动作产出数据，None 如果无输出
async fn execute_utility_action(action: &ContainerAction) -> Result<Option<Value>> {
    match action.action_type.as_str() {
        "download" => {
            let save_dir = action
                .config
        ...
        _ => unreachable!("execute_utility_action called with unsupported type: {}", action.action_type),
    }
}
...
async fn execute_actions(
    ...
    for action in &config.actions {
        ...
        let output = match action.action_type.as_str() {
            // 导航类
            "navigate" | "back" | "forward" | "reload" | "new_page" | "close_page" | "switch_page" => {
                execute_nav_action(action).await?
            }
            // 交互类
            "click" | "double_click" | "input" | "fill" | "select" | "check" | "hover"
            | "keyboard" | "drag_to" | "context_menu" | "scroll" | "scroll_to_element" => {
                execute_interact_action(action, &resolved_inputs).await?
            }
            // 提取类
            "extract" | "extract_table" | "extract_links" | "get_title" | "current_url"
            | "pages" | "snapshot" | "screenshot" | "cookies" | "evaluate" => {
                execute_extract_action(action).await?
            }
            // 等待类
            "wait" | "wait_network_idle" | "wait_load_state" | "wait_url_contains" => {
                execute_wait_action(action).await?
            }
            // 工具类
            "download" | "upload" | "pdf" | "set_headers" | "switch_frame"
            | "handle_dialog" | "verify" => {
                execute_utility_action(action).await?
            }
            _ => {
                return Err(anyhow!(
        ...
        if let Some(value) = output {
            output_ports.insert(action.id.clone(), value);
        }
