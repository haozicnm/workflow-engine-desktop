...
impl NodeExecutor for ConvertToNumberNode {
    async fn execute(
        ...
        if let Ok(i) = text.parse::<i64>() {
            return Ok(serde_json::json!({ "result": i }));
        }
        if let Ok(f) = text.parse::<f64>() {
            return Ok(serde_json::json!({ "result": f }));
        }
...
impl NodeExecutor for ConvertToJsonNode {
    async fn execute(
        ...
        let text = input
            .as_str()
            .ok_or_else(|| anyhow!("to_json 需要字符串输入"))?;
        let parsed: serde_json::Value =
            serde_json::from_str(text).map_err(|e| anyhow!("JSON 解析失败: {}", e))?;
        info!("JSON 解析成功");
...
impl NodeExecutor for ConvertToCsvNode {
    async fn execute(
        ...
        if arr.is_empty() {
            return Ok(serde_json::json!({ "result": "", "row_count": 0 }));
        }
        ...
        Ok(
            serde_json::json!({ "result": csv.trim_end(), "row_count": arr.len(), "column_count": columns.len(), "columns": columns }),
        )
    }
}
...
impl NodeExecutor for ConvertToBase64Node {
    async fn execute(
        ...
        let result = match input {
            ...
            other => {
                let json =
                    serde_json::to_string(other).map_err(|e| anyhow!("序列化失败: {}", e))?;
