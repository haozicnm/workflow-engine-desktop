        ...
        serde_json::Value::String(ref_str) => {
            if let Some(path) = ref_str.strip_prefix("output.") {
                ...
                if let Some(f) = field {
                    ...
                    val.as_array().cloned().ok_or_else(|| anyhow!("字段 '{}' 不是数组", f))
                } else {
                    output.as_array().cloned().ok_or_else(|| anyhow!("步骤输出不是数组: step_{}", step_id))
                }
            } else {
                ctx.variables.get(ref_str)
                    .and_then(|v| v.as_array()).cloned()
                    .or_else(|| {
                        ctx.variables.get(ref_str).and_then(|v| if v.is_object() { Some(vec![v.clone()]) } else { None })
                    })
...
fn extract_field<'a>(item: &'a serde_json::Value, field: Option<&str>) -> &'a serde_json::Value {
    match field {
        Some(f) => {
            ...
            for part in f.split('.') {
                match current.get(part) { Some(v) => current = v, None => return &serde_json::Value::Null }
            }
            current
        }
        None => item,
...
fn compare_values(a: &serde_json::Value, b: Option<&serde_json::Value>, op: &str) -> bool {
    let b = match b { Some(v) => v, None => return false };
    if let (Some(sa), Some(sb)) = (a.as_str(), b.as_str()) {
        return match op {
            "==" => sa == sb, "!=" => sa != sb, "contains" => sa.contains(sb),
            "starts_with" => sa.starts_with(sb), "ends_with" => sa.ends_with(sb), _ => false,
        };
    }
    if let (Some(na), Some(nb)) = (a.as_f64(), b.as_f64()) {
        return match op {
            "==" => (na - nb).abs() < f64::EPSILON, "!=" => (na - nb).abs() >= f64::EPSILON,
            ">" => na > nb, ">=" => na >= nb, "<" => na < nb, "<=" => na <= nb, _ => false,
        };
    }
    if let (Some(ba), Some(bb)) = (a.as_bool(), b.as_bool()) {
        return match op { "==" => ba == bb, "!=" => ba != bb, _ => false };
    }
    match op { "==" => *a == *b, "!=" => *a != *b, _ => false }
}

fn eval_condition(item: &serde_json::Value, condition: &serde_json::Value, field: Option<&str>, op: &str) -> bool {
    let item_val = if let Some(f) = field {
        let mut current = item;
        for part in f.split('.') { match current.get(part) { Some(v) => current = v, None => return false } }
        current
    } else { item };
    match op {
        "is_null" => item_val.is_null(), "is_not_null" => !item_val.is_null(),
        "is_empty" => matches!(item_val, serde_json::Value::String(s) if s.is_empty()) || matches!(item_val, serde_json::Value::Array(a) if a.is_empty()) || item_val.is_null(),
...
fn compare_json(a: &serde_json::Value, b: &serde_json::Value) -> std::cmp::Ordering {
    ...
    match (a, b) {
        (serde_json::Value::Number(na), serde_json::Value::Number(nb)) => na.as_f64().partial_cmp(&nb.as_f64()).unwrap_or(Ordering::Equal),
...
impl NodeExecutor for ArrayFilterNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        ...
        let source = resolve_array_source(config.get("source").unwrap_or(&serde_json::Value::Null), ctx)?;
        let condition = config.get("condition").ok_or_else(|| anyhow!("filter 缺少 condition 参数"))?;
        ...
        Ok(serde_json::json!({ "source_count": source.len(), "result_count": result.len(), "result": result }))
    }
}
...
impl NodeExecutor for ArraySortNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        ...
        Ok(serde_json::json!({ "source_count": source.len(), "field": field, "order": order, "result": result }))
    }
}
...
impl NodeExecutor for ArrayDedupNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        ...
        Ok(serde_json::json!({ "source_count": source.len(), "result_count": result.len(), "field": field, "result": result }))
    }
}
...
impl NodeExecutor for ArrayPaginateNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        let config = &step.config;
        let source = resolve_array_source(config.get("source").unwrap_or(&serde_json::Value::Null), ctx)?;
        let page = config.get("page").and_then(|v| v.as_u64()).unwrap_or(1).max(1) as usize;
        let page_size = config.get("page_size").and_then(|v| v.as_u64()).unwrap_or(10).max(1) as usize;
        ...
        Ok(serde_json::json!({ "page": page, "page_size": page_size, "total": total, "total_pages": total_pages, "count": page_items.len(), "result": page_items }))
    }
}
...
impl NodeExecutor for ArrayMapNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        ...
        Ok(serde_json::json!({ "source_count": source.len(), "template": template, "result": result }))
    }
}

fn apply_map_template(template: &str, item: &serde_json::Value, index: usize, fields: Option<&[&str]>) -> serde_json::Value {
    if template.trim() == "{{__item}}" { return item.clone(); }
    if template.trim() == "{{__index}}" { return serde_json::json!(index); }
    ...
    if let Some(field_list) = fields {
        if !field_list.is_empty() {
            ...
            for f in field_list { obj.insert(f.to_string(), extract_field(item, Some(f)).clone()); }
...
impl NodeExecutor for ArrayJoinNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        ...
        let parts: Vec<String> = source.iter().map(|item| {
            let val = match field { Some(f) => extract_field(item, Some(f)), None => item };
            match val { serde_json::Value::String(s) => s.clone(), serde_json::Value::Null => String::new(), other => other.to_string() }
        }).collect();

        let result = parts.join(separator);
        Ok(serde_json::json!({ "source_count": source.len(), "separator": separator, "field": field, "length": result.len(), "result": result }))
    }
}
...
impl NodeExecutor for ArrayReduceNode {
    async fn execute(&self, step: &Step, ctx: &mut ExecutionContext, _executor: &Arc<StepExecutor>) -> Result<serde_json::Value> {
        let config = &step.config;
        let source = resolve_array_source(config.get("source").unwrap_or(&serde_json::Value::Null), ctx)?;
        let aggregator = config.get("aggregator").and_then(|v| v.as_str()).unwrap_or("count");
        let field = config.get("field").and_then(|v| v.as_str());

        let result = match aggregator {
            "count" => serde_json::json!(source.len()),
            "sum" => {
                let sum: f64 = source.iter().filter_map(|item| {
            ...
            "avg" => {
                let vals: Vec<f64> = source.iter().filter_map(|item| {
                    let val = extract_field(item, field);
                    val.as_f64().or_else(|| val.as_str().and_then(|s| s.parse::<f64>().ok()))
                }).collect();
            ...
            "min" => {
                let min = source.iter().filter_map(|item| {
                    let val = extract_field(item, field);
                    val.as_f64().or_else(|| val.as_str().and_then(|s| s.parse::<f64>().ok()))
            ...
            "max" => {
                let max = source.iter().filter_map(|item| {
                    let val = extract_field(item, field);
                    val.as_f64().or_else(|| val.as_str().and_then(|s| s.parse::<f64>().ok()))
            ...
            _ => return Err(anyhow!("未知的聚合操作: {}（支持 count/sum/avg/min/max/first/last）", aggregator)),
        };

        info!("数组聚合: {} → {:?}, {} 条", aggregator, field, source.len());
        Ok(serde_json::json!({ "aggregator": aggregator, "field": field, "source_count": source.len(), "result": result }))
    }
}
